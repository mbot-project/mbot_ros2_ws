...
def generate_launch_description():
    ...
    # 4. Define the photo capture node
    photo_capture_node = Node(
        package='mbot_vision',
        executable='apriltag_photo_capture',
        name='apriltag_photo_capture',
        parameters=[{
            'save_directory': '~/mbot_ws/src/mbot_vision/media',
                    ...
                    RegisterEventHandler(
                        event_handler=OnProcessStart(
                            ...
                            on_start=[
                                ...
                                # Start photo capture node after apriltag node
                                RegisterEventHandler(
                                    event_handler=OnProcessStart(
                                        target_action=apriltag_node,
                                        on_start=[photo_capture_node]
                                    )
                                )
                            ]
                        )
